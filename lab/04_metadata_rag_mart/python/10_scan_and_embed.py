...
TABLE_DETAIL_BI = {
    ...
    "financial_news": "Market-moving news from Bloomberg, Reuters, AWP (Swiss wire), FT, NZZ. Covers equities, fixed income, macro. sentiment_score is ML-generated (-1 to 1). related_isins links to instruments. Key for event-driven analysis and position monitoring.",
    "public_filings": "SIX Exchange ad-hoc disclosures and regulatory filings (FINMA, BaFin, SEC). Includes annual reports, interim results, share buybacks. financial_impact flags material disclosures. Key for fundamental analysis.",
    "analyst_recommendations": "Sell-side analyst research notes and ratings. Covers Swiss, EU, US equities. recommendation changes (upgrades/downgrades) are signals for position review. target_price vs current_price shows implied upside.",
    "financial_reports": "Quarterly and annual earnings reports. revenue/EPS surprise data for event analysis. guidance changes are key signals. Links to instruments via company_isin.",
}
...
COLUMN_DETAIL_BI = {
    ...
    ("financial_news", "sentiment_score"): "ML-generated sentiment score from -1.0 (very negative) to 1.0 (very positive). Used for event-driven trading signals and portfolio risk monitoring.",
    ("financial_news", "impact_level"): "News impact classification: low (routine), medium (sector-level), high (stock-moving), critical (portfolio-wide action required).",
    ("financial_news", "related_isins"): "JSON array of ISINs mentioned in the article. Links to instruments table for cross-referencing with positions.",
    ("financial_news", "related_sectors"): "JSON array of GICS sectors affected by the news. Used for sector-level exposure analysis.",
    ("public_filings", "financial_impact"): "Regulatory assessment: positive, negative, neutral, or material. 'material' triggers ad-hoc disclosure requirements under SIX rules.",
    ("public_filings", "key_figures"): "JSON object with key financial metrics from the filing (revenue, net_income, capital_requirement, etc.).",
    ("analyst_recommendations", "recommendation"): "Current rating: strong_buy, buy, hold, sell, strong_sell. Rating changes (vs prev_recommendation) are key signals for portfolio review.",
    ("analyst_recommendations", "prev_recommendation"): "Previous rating. Comparison with current recommendation identifies upgrades/downgrades — key broker signals.",
    ("analyst_recommendations", "upside_pct"): "Implied upside/downside: (target_price / current_price - 1) × 100. Negative values signal downside risk.",
    ("financial_reports", "revenue_surprise_pct"): "Revenue vs consensus: positive = beat, negative = miss. Misses >2% typically trigger position review.",
    ("financial_reports", "eps_surprise_pct"): "EPS vs consensus: positive = beat, negative = miss. Combined with revenue surprise for earnings quality assessment.",
    ("financial_reports", "guidance"): "Forward guidance: raised, maintained, lowered, withdrawn. 'lowered' or 'withdrawn' are negative signals requiring position review.",
}

# Classification rules
RESTRICTED_TABLES = {"aml_alerts", "compliance_checks"}
CONFIDENTIAL_TABLES = {"risk_metrics", "risk_limits", "orders", "executions", "analyst_recommendations"}
INTERNAL_TABLES = {"positions", "cash_balances", "market_prices", "clients", "accounts", "financial_news"}
PUBLIC_TABLES = {"exchanges", "instruments", "counterparties", "public_filings", "financial_reports"}
...
RELATIONSHIPS = [
    ...
    ("financial_news", "related_isins", "instruments", "isin", "many_to_many"),
    ("public_filings", "issuer_isin", "instruments", "isin", "one_to_many"),
    ("analyst_recommendations", "instrument_isin", "instruments", "isin", "one_to_many"),
    ("financial_reports", "company_isin", "instruments", "isin", "one_to_many"),
]
...
KPI_PATTERNS = [
    ...
    {"kpi_name":"Market Sentiment by Sector","kpi_description":"News sentiment aggregated by sector for exposure decisions. Combines financial news sentiment scores with sector classification to identify sectors under positive or negative pressure.","domain":"broker_intelligence","required_tables":["financial_news","instruments"],"required_columns":["financial_news.sentiment_score","financial_news.related_sectors","financial_news.impact_level","financial_news.category"],"sql_template":"SELECT fn.related_sectors, AVG(CAST(fn.sentiment_score AS FLOAT)) AS avg_sentiment, COUNT(*) AS news_count, SUM(CASE WHEN fn.impact_level IN ('high','critical') THEN 1 ELSE 0 END) AS high_impact_count FROM lake.financial_news fn GROUP BY fn.related_sectors ORDER BY avg_sentiment ASC","classification":"internal"},
    {"kpi_name":"Analyst Consensus Changes","kpi_description":"Recent analyst upgrades and downgrades correlated with held positions. Identifies instruments where sell-side consensus is shifting, enabling proactive position review.","domain":"broker_intelligence","required_tables":["analyst_recommendations","instruments","positions"],"required_columns":["analyst_recommendations.recommendation","analyst_recommendations.prev_recommendation","analyst_recommendations.instrument_isin","analyst_recommendations.upside_pct","positions.market_value"],"sql_template":"SELECT ar.instrument_name, ar.recommendation, ar.prev_recommendation, ar.target_price, ar.current_price, ar.upside_pct, ar.analyst_firm FROM lake.analyst_recommendations ar WHERE ar.recommendation != ar.prev_recommendation ORDER BY ar.published_date DESC","classification":"confidential"},
    {"kpi_name":"Earnings Surprise Impact","kpi_description":"Revenue and EPS misses matched with client position exposure. Identifies which clients are affected by earnings disappointments and quantifies potential portfolio impact.","domain":"broker_intelligence","required_tables":["financial_reports","instruments","positions","accounts","clients"],"required_columns":["financial_reports.revenue_surprise_pct","financial_reports.eps_surprise_pct","financial_reports.guidance","positions.market_value","clients.client_code"],"sql_template":"SELECT fr.company, fr.period, fr.revenue_surprise_pct, fr.eps_surprise_pct, fr.guidance, fr.key_takeaway FROM lake.financial_reports fr WHERE fr.revenue_surprise_pct < -1.0 OR fr.guidance IN ('lowered','withdrawn') ORDER BY fr.report_date DESC","classification":"confidential"},
    {"kpi_name":"Regulatory Filing Alerts","kpi_description":"Material regulatory filings matched with held instruments. Flags ad-hoc disclosures, profit warnings, and material filings for instruments in client portfolios.","domain":"broker_intelligence","required_tables":["public_filings","instruments","positions"],"required_columns":["public_filings.financial_impact","public_filings.filing_type","public_filings.issuer_isin","positions.market_value","instruments.isin"],"sql_template":"SELECT pf.issuer, pf.filing_type, pf.title, pf.financial_impact, pf.summary FROM lake.public_filings pf WHERE pf.financial_impact IN ('negative','material') ORDER BY pf.filing_date DESC","classification":"public"},
]
